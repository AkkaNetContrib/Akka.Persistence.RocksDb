...
{
    public class RocksDbCurrentPersistenceIdsSpec : CurrentPersistenceIdsSpec
    {
        ...
        public static Config Config(int id) => ConfigurationFactory.ParseString($@"
            ...
            akka.persistence.journal.rocksdb {{
                ...
                auto-initialize = on
                path = rocks_cpid_{id}.db
            }}
            akka.test.single-expect-default = 3s")
            .WithFallback(RocksDbReadJournal.DefaultConfiguration());

        public RocksDbCurrentPersistenceIdsSpec(ITestOutputHelper output) 
            : base(Config(Counter.GetAndIncrement()), nameof(RocksDbCurrentPersistenceIdsSpec), output)
        {
